...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS loans (
    ...
    status TEXT DEFAULT 'Pending', -- 'Pending', 'Active', 'Paid', 'Rejected'
...
if (!hasPeriod) db.exec("ALTER TABLE transactions ADD COLUMN period TEXT");

const hasMonth = tableInfo.some((col: any) => col.name === 'month');
if (!hasMonth) db.exec("ALTER TABLE transactions ADD COLUMN month TEXT");
...
if (!hasBorrowerName) db.exec("ALTER TABLE loans ADD COLUMN borrower_name TEXT");

const hasMonths = loanTableInfo.some((col: any) => col.name === 'months');
if (!hasMonths) db.exec("ALTER TABLE loans ADD COLUMN months INTEGER DEFAULT 1");
...
async function startServer() {
  ...
  app.post("/api/members", (req, res) => {
    ...
    if (existing) return res.status(400).json({ error: "A member with this name already exists." });
  ...
  app.get("/api/members/:id", (req, res) => {
    ...
    memberLoans.forEach((loan: any) => {
      const totalInt = loan.principal * loan.interest_rate * loan.months;
      const totalToPay = loan.principal + totalInt;
  ...
  app.post("/api/contributions", (req, res) => {
    ...
    const member = db.prepare("SELECT slots FROM members WHERE id = ?").get(member_id) as any;
    if (!member) return res.status(404).json({ error: "Member not found" });
  ...
  app.post("/api/loans", (req, res) => {
    ...
    if (loanAmount <= 0) return res.status(400).json({ error: "Loan amount must be greater than zero." });
    if ((loanAmount + currentDebt) > totalEligibility) {
      return res.status(400).json({ 
        error: `Loan exceeds eligibility cap. Total limit: ₱${totalEligibility.toLocaleString()} (${borrowerId ? `2x your principal: ₱${(borrowerPrincipal * 2).toLocaleString()} + ` : ''}guarantor principal: ₱${guarantorPrincipal.toLocaleString()}). Current active debt: ₱${currentDebt.toLocaleString()}. You can borrow up to ₱${(totalEligibility - currentDebt).toLocaleString()} more.` 
      });
    }
  ...
  app.post("/api/loans/:id/approve", (req, res) => {
    const { id } = req.params;
    try {
      db.prepare("UPDATE loans SET status = 'Active' WHERE id = ? AND status = 'Pending'").run(id);
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.post("/api/loans/:id/reject", (req, res) => {
    const { id } = req.params;
    try {
      db.prepare("UPDATE loans SET status = 'Rejected' WHERE id = ? AND status = 'Pending'").run(id);
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
